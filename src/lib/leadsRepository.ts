...
const toLead = (row: DbLead): Lead => ({
  ...
  createdAt: new Intl.DateTimeFormat('uk-UA', { day:'2-digit', month:'2-digit', hour:'2-digit', minute:'2-digit' }).format(new Date(row.created_at)),
});
...
export async function bulkCreateLeads(incoming: Lead[], current: Lead[]): Promise<{ created: Lead[]; skipped: number }> {
  const existingPhones = new Set(current.map(item => item.phone.replace(/\D/g, '')).filter(Boolean));
  const unique: Lead[] = [];
  let skipped = 0;
  for (const lead of incoming) {
    const normalized = lead.phone.replace(/\D/g, '');
    if (!normalized || existingPhones.has(normalized)) { skipped++; continue; }
    existingPhones.add(normalized);
    unique.push(lead);
  }
  if (!unique.length) return { created: [], skipped };
  if (!isSupabaseConfigured || !supabase) {
    const created = unique.map(lead => ({ ...lead, id: crypto.randomUUID(), createdAt: lead.createdAt || 'Імпортовано' }));
    localSave([...created, ...current]);
  ...
  const rows = unique.map(lead => ({
    owner_id: auth.user!.id, name: lead.name, phone: lead.phone, whatsapp: lead.whatsapp || null,
    service: lead.service, source: lead.source, campaign: lead.campaign, status: lead.status,
    value: lead.value, next_action: lead.nextAction || null, note: lead.note || null, form_answers: lead.formAnswers || null,
    lost_reason: lead.lostReason || null, external_id: lead.externalId || null
  }));
...
function extractServiceFromAnswers(text: string | undefined) {
  ...
  for (const line of text.split('\n')) {
    ...
    const excluded = ['чи запускали','скільки вакансій','пошуку кандидатів','скільки працівників','бюджет'];
    const accepted = ['які послуги ви рекламуєте','які послуги','яку послугу','яка послуга','що ви рекламуєте','що потрібно рекламувати','напрямок бізнесу','ваша ніша','чим займаєтесь'];
    if (answer && !excluded.some(token => question.includes(token)) && accepted.some(token => question.includes(token))) return answer;
  }
  return '';
}
...
function serviceLooksLikeQuestion(value: string) {
  ...
  return normalized.includes('чи запускали') || normalized.includes('запускали рекламу') || normalized.includes('скільки вакансій') || normalized.includes('пошуку кандидатів') || normalized.includes('питання');
}
...
export async function repairImportedLeads(current: Lead[]): Promise<{ leads: Lead[]; repaired: number }> {
  const changed = current.map(lead => {
    ...
    const needsServiceRepair = (serviceLooksLikeQuestion(lead.service) || lead.service === 'Meta Ads' || lead.service === 'Не визначено') && Boolean(inferredService);
    const needsAnswersRepair = !lead.formAnswers && Boolean(lead.note) && lead.note.split('\n').some(line => line.includes(':'));
