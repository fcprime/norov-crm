...
const archiveReasons = ['Завершили співпрацю','Клієнт поставив рекламу на паузу','Немає бюджету','Не задоволений результатом','Перейшов до іншого спеціаліста','Послуга більше не потрібна','Інше'];
...
function App() {
  ...
  async function setClientStatus(client:Client,status:ClientStatus){
    if(status==='archived'){ setArchivingClient(client); return; }
    const patch:Client={...client,status, archivedAt:undefined, archiveReason:undefined};
    await saveClientItem(patch);
    if(client.status==='archived'&&status==='active') setNotice(`${client.name}: повернуто в активні. Перевірте дату наступної оплати.`);
  }
  async function archiveClient(client:Client, archivedAt:string, reason:string){
    setSyncing(true);setError('');
    try{
      const saved=await saveClient({...client,status:'archived',archivedAt,archiveReason:reason},clients);
      setClients(prev=>prev.map(c=>c.id===saved.id?saved:c));
      setArchivingClient(null);
      setNotice(`${saved.name}: перенесено в архів${reason?` — ${reason}`:''}.`);
  ...
  const reminderClients=activeClients.filter(c=>daysDiff(c.nextPaymentDate)<=Math.max(0,c.reminderDaysBefore??3)).sort((a,b)=>daysDiff(a.nextPaymentDate)-daysDiff(b.nextPaymentDate));
  ...
  useEffect(()=>{
    if(typeof Notification==='undefined'||Notification.permission!=='granted'||!reminderClients.length)return;
    const key=`norov-reminder-${today()}`;
    if(localStorage.getItem(key))return;
    const overdueCount=reminderClients.filter(c=>daysDiff(c.nextPaymentDate)<0).length;
    const todayCount=reminderClients.filter(c=>daysDiff(c.nextPaymentDate)===0).length;
    const soonCount=reminderClients.filter(c=>daysDiff(c.nextPaymentDate)>0).length;
    new Notification('NOROV CRM — оплати',{body:[overdueCount?`Прострочено: ${overdueCount}`:'',todayCount?`Сьогодні: ${todayCount}`:'',soonCount?`Незабаром: ${soonCount}`:''].filter(Boolean).join(' · ')});
    localStorage.setItem(key,'1');
  },[reminderClients.length]);

  async function enableBrowserReminders(){
    if(typeof Notification==='undefined'){setError('Цей браузер не підтримує системні сповіщення.');return;}
    const permission=await Notification.requestPermission();
    setNotice(permission==='granted'?'Сповіщення браузера увімкнено.':'Дозвіл на сповіщення не надано.');
  }
  ...
  return <div className="app-shell">
    ...
    <main>
      <header className="topbar"><div className="search"><Search size={18}/><input value={query} onChange={e=>setQuery(e.target.value)} placeholder={`Пошук: ${pageTitle.toLowerCase()}...`}/><kbd>⌘ K</kbd></div><div className="top-actions"><span className={`sync-badge ${syncing?'syncing':''}`}>{syncing?'Збереження…':isSupabaseConfigured?'База підключена':'Локальний режим'}</span><div className="reminder-wrap"><button className="icon-btn" title="Нагадування про оплати" onClick={()=>setRemindersOpen(v=>!v)}><Bell size={19}/>{reminderClients.length>0&&<i/>}{reminderClients.length>0&&<b>{reminderClients.length}</b>}</button>{remindersOpen&&<ReminderCenter clients={reminderClients} onClose={()=>setRemindersOpen(false)} onOpenClient={c=>{setEditingClient(c);setPage('clients');setRemindersOpen(false)}} onPay={c=>{setPayingClient(c);setRemindersOpen(false)}} onEnableBrowser={()=>void enableBrowserReminders()}/>}</div><div className="avatar">SN</div><div className="user"><strong>Serhii Norov</strong><span>Administrator</span></div><button className="signout" title="Вийти" onClick={()=>supabase?.auth.signOut()}><LogOut size={17}/></button></div></header>
      <section className="content">
        ...
        {loading?<div className="loading-state">Завантажуємо CRM…</div>:
          page==='dashboard'?<Dashboard clients={clients} payments={payments} overdue={overdue} dueToday={dueToday} due7={due7} onPay={setPayingClient} onOpenClients={()=>setPage('clients')}/>:page==='leads'?<>
            ...
            {view==='kanban'?<Kanban leads={filteredLeads} clients={clients} onEdit={setEditingLead} onMove={moveLead} onAdd={status=>setEditingLead({...emptyLead,status})}/>:<LeadTable leads={filteredLeads} clients={clients} onEdit={setEditingLead}/>} </>:page==='clients'?<ClientsPage clients={filteredClients} allClients={clients} payments={payments} filter={clientFilter} setFilter={setClientFilter} onAdd={()=>setEditingClient({...emptyClient})} onEdit={setEditingClient} onPay={setPayingClient} onStatus={setClientStatus} onDelete={setDeletingClient}/>:<PaymentsPage rows={paymentRows} allPayments={payments} clients={clients}/>
        }
      </section>
    </main>
...
function ArchiveClientModal({client,onClose,onConfirm}:{client:Client;onClose:()=>void;onConfirm:(c:Client,date:string,reason:string)=>void}){
  const [archivedAt,setArchivedAt]=useState(today());
  const [reason,setReason]=useState('Завершили співпрацю');
  const [customReason,setCustomReason]=useState('');
  const finalReason=reason==='Інше'?customReason.trim():reason;
  return <div className="modal-backdrop"><div className="modal archive-modal"><div className="modal-head"><div><span>Архівація клієнта</span><h2>{client.name}</h2></div><button onClick={onClose}><X size={20}/></button></div><div className="archive-note"><Archive size={19}/><div><strong>Історія не видаляється</strong><span>Оплати, LTV, контакти та нотатки залишаться в CRM. Клієнта можна буде повернути в активні.</span></div></div><div className="form-grid one"><label>Дата завершення співпраці<input type="date" value={archivedAt} onChange={e=>setArchivedAt(e.target.value)}/></label><label>Причина<select value={reason} onChange={e=>setReason(e.target.value)}>{archiveReasons.map(r=><option key={r}>{r}</option>)}</select></label>{reason==='Інше'&&<label>Вкажіть причину<textarea rows={3} value={customReason} onChange={e=>setCustomReason(e.target.value)} placeholder="Короткий коментар"/></label>}</div><div className="archive-summary"><span>Після архівації нагадування про оплату для цього клієнта припиняться.</span></div><div className="modal-actions"><button className="secondary" onClick={onClose}>Скасувати</button><button className="danger-button" disabled={!archivedAt||!finalReason} onClick={()=>onConfirm(client,archivedAt,finalReason)}><Archive size={17}/>Архівувати</button></div></div></div>
}
